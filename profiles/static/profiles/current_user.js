    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if(num_posts >= response.length) {
                ...
                endBlock.textContent = 'Keine weiteren Beiträge vorhanden.';
        ...
        success: function(response) {
            ...
            <div class="align-none user-detials mt-2 ml-2">
                ...
                <form class="follow-unfollow-form" user_id='${data.profile_id}'>
                    <button class="btn-outline text-muted follow-btn" id="follow-btn" >${data.is_following ? 'Nicht mehr Folgen' : 'Folgen'}</button>
                </form>
            </div>
