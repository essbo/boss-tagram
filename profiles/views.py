...
def register_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, 'Dein Account wurde erstellt!')
...
def edit_profile_view(request, username):
    ...
    if request.method == 'POST':
        ...
        if u_form.is_valid() and p_form.is_valid():
            u_form.save()
            p_form.save()
            messages.success(request, 'Dein Profil wurde erfolgreich aktualisiert!')
            return redirect('user_profile')
...
def follow_unfollow_view(request):
    ...
    if(is_following.exists()):
        ...
        return JsonResponse({'Folgen': False, 'followers': followers})
    else:
        Follower.objects.create(user=user, followed_by=request.user)
        followers = Follower.objects.filter(user=user).all().count()
        return JsonResponse({'Folgen': True, 'followers': followers})


def search_view(request):
    return render(request, 'profiles/search.html', context={'title': 'Suchen'})
