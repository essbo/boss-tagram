...
def post_delete_view(request, pk):
    ...
    if(post.author == request.user.profile):
        return redirect('home')
    post.delete()
    messages.success(request, 'Dein Post wurde erfolgreich gelöscht!')
    return redirect('home')
...
def comment_view(request):
    ...
    if request.META.get('HTTP_X_REQUESTED_WITH') == 'XMLHttpRequest':
        ...
        comment.save()
        commented = True
        return JsonResponse({'Kommentiert': commented, 'no_of_comments': post.no_of_comments, 'user_id': request.user.id, 'user_name': request.user.username, 'profile_pic': request.user.profile.image.url, 'time': '0 seconds ago', 'comment_content': comment_data})
...
def new_post(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            post.save()
            messages.success(request, 'Neuer Post wurde erstellt!')
            return redirect('home')
    else:
        form = NewPostForm()
    context = {
...
def post_update_view(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, 'Dein Post wurde geupdatet!')
            return redirect('home')
